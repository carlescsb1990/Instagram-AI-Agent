...
class RionaAIDashboard {
    ...
    setupEventListeners() {
        ...
        // Content generation
        const generateBtn = document.querySelector('[onclick="dashboard.generateAIContent()"]');
        if (generateBtn) {
    ...
    showPage(pageName) {
        ...
        // Add to activity log
        this.addLogEntry('info', `Navegando a ${this.getPageDisplayName(pageName)}`);
    }
    ...
    async loadInitialData() {
        try {
            ...
            this.addLogEntry('error', `Error cargando datos iniciales: ${error.message}`);
        } finally {
    ...
    async loadPageData(pageName) {
        try {
            ...
            this.addLogEntry('error', `Error cargando datos de ${pageName}: ${error.message}`);
        } finally {
            this.setLoading(false);
        }
    }
    ...
    async loadDashboardMetrics() {
        try {
            ...
            if (response && response.success) {
                ...
                this.updateElement('activeAccounts', response.data.activeAutomations || 0);
            }
        } catch (error) {
            console.warn('Could not load dashboard metrics from API, using localStorage:', error);
    ...
    async generateAIContent() {
        try {
            const contentType = document.getElementById('contentType')?.value || 'comment';
            const context = document.getElementById('contentContext')?.value || '';
            const character = localStorage.getItem('selectedCharacter') || 'arcane-edge';

            if (!context.trim()) {
                this.addLogEntry('warning', 'Por favor proporciona un contexto para generar contenido');
                return;
            }
    ...
    copyContent() {
        const contentOutput = document.getElementById('contentOutput');
        if (contentOutput) {
            navigator.clipboard.writeText(contentOutput.textContent).then(() => {
    ...
    toggleFeature(toggleElement) {
        ...
        this.addLogEntry('info', `${featureName} ${isActive ? 'activado' : 'desactivado'}`);
    }
    ...
    async runAllAutomations() {
        try {
            ...
            const response = await this.apiCall('/social/instagram/automation', 'POST', {});
    ...
    renderAnalytics(data) {
        if (data.metrics) {
            this.updateElement('totalLikes', data.metrics.totalLikes?.toLocaleString() || '0');
            this.updateElement('totalComments', data.metrics.totalComments?.toLocaleString() || '0');
            this.updateElement('totalFollows', data.metrics.totalFollows?.toLocaleString() || '0');
    ...
    updateActivityFeed() {
        ...
        feedElement.innerHTML = this.activityLog.slice(0, 10).map(entry => `
            <div class="activity-item">
  ...
  async apiCall(endpoint, method = "GET", data = null) {
    try {
      ...
      if (jsonEnd > 0 && jsonEnd < cleanText.length) {
        console.warn(`Trimming response from ${cleanText.length} to ${jsonEnd} characters`);
      ...
      try {
        ...
        console.error('Response text (last 50 chars):', text.substring(Math.max(0, text.length - 50)));
    ...
    getMockResponse(endpoint, method, data) {
        ...
        const mockKey = Object.keys(mockResponses).find(key => endpoint.includes(key));
        return mockKey ? mockResponses[mockKey] : { success: false, error: 'Mock endpoint not found' };
    }
    ...
    async runDebugTest() {
        ...
        const endpoints = ['/api/health', '/api/users', '/api/accounts', '/api/social'];
        ...
        for (const endpoint of endpoints) {
            try {
                ...
                const text = await response.text();
                console.log(`✅ ${endpoint}: Status ${response.status}, Length ${text.length}`);
                ...
                try {
                    ...
                    this.addLogEntry('warning', `${endpoint} - Response not JSON: ${parseError.message}`);
                }
    ...
    testLocalStorage() {
        try {
            ...
            const users = this.getStoredUsers();
            const accounts = this.getStoredAccounts();
            this.addLogEntry('info', `Datos: ${users.length} usuarios, ${accounts.length} cuentas`);
